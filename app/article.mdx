...
A procedural generation algorithm it's what I'm looking for.
...
When facing such a broad problem, the typical first step is to shoot the messenger with a ton of questions:
...
The infinitely many roads are all facing us and taking the first step could feel daunting. Simply staring at them won't get us anywhere though, so let's just pick one without the fear of retracing our steps.
...
Let's take numbers for instance. They were first introduced to keep track of quantity. But how many fingers on our hands, liters of fuel needed for a trip to Mars, or the ratio between the circumference and the radius of a circle, are all widely different use cases.
Extracting common rules and concepts from different instances is called an abstraction.
As numbers serve as an essential abstraction for counting, similarly, we need a shared descriptor of plants for our drawings.
...
In practice, we can take some pictures of real plants we'd like to draw and literally blur the details by... blurring the pictures. Then we can toggle on and off different elements to see their role in defining the blurred picture.
...
What's left is the branching structure, thus it's the most impacting element of the picture.   
We're also not interested in any kind of color or thickness for now, so we can get rid of those elements too by representing everything with straight lines. Nice! Let's focus on that then.
</Prose>
...
Let's take some pictures of plants and observe them with from the eyes of a scientist.
...
<Prose>
Okay, despite the differences, our abstractions all look similar.
...
If we associate this base element to the symbol T we get a weird definition:   
T = ½ T + ½ T    
...
To unclutter the back-and-forth we could keep track of the turtle's movement using a stack: a way of organizing data that, just like a stack of plates, lets you add and remove elements only from the top.
...
What will happen at a high level will be:
 - **[**: store the current position and orientation.
 - **]**: move up the pen, retrieve the last stored position and orientation, retrace your steps and orientation, move down the pen.
...
We can draw the result using our trusted turtle and here it is.

</Prose>
<TurtleSketch allowedCommands={toCmdSeq('Ff+-T[]')} defaultSeq={toCmdSeq("F[+F[+F[+T][-T]][-F[+T][-T]]][-F[+F[+T][-T]][-F[+T][-T]]]")} />
...
What we just described is a Lindenmayer System. It get's its name from the bright botanist who first came up with it, Aristid Lindenmayer.   
This kind of structures, commonly abbreviated to L-systems, are an example of a broader class of computational objects: formal grammars.
...
- **Paolo Ginefra**:<RoleList roles={['Applet Development', 'Front End', 'Project Management', 'Research', 'Writing']}/>
</Prose>
</Toggleable>
